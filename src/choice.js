...
function Choice(props){
  ...
  const [pos, set] = useSpring(() => ({
    x: 20*-1*(props.index + count),
    y: 0,  
    scale: 1, 
    rot: 0
  }))
  var offset = 20*-1*(props.index + count)
  var offset = 0
  ...
  useEffect(()=> {
    ...
    console.log(count)
    set({ x: isGone ? (500 + window.innerWidth)*side : offset, scale: 1
        })
  ...
  const bind = useDrag(({ args: [index], down, movement: [mx,my], direction: [xDir], velocity }) => {
    ...
    set(() => { 
      const x = isGone ? (200 + window.innerWidth) * dir : down ? mx : offset //cards shift to replace removed top card
